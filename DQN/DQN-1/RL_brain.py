...
class DeepQNetwork:
    def __init__(
            ...
            output_graph=False):
        self.n_actions = n_actions # 可选择的动作
        self.n_features = n_features  # Observation的情况，可理解为state
        self.lr = learning_rate
        self.gamma = reward_decay
        self.epsilon_max = e_greedy  # epsilon 的最大值
        self.replace_target_iter = replace_target_iter  # 更换 target_net（用eval_net的参数去替换target_net的参数）的步数
        self.memory_size = memory_size  # 记忆上限(记忆库的容量:可以记录了多少条transition的数据)
        ...
        # 初始化全 0 记忆 [s, a, r, s_]；n_features*2+2是因为s和S_有n_features个，每次选取得action为1，reward为1
        ...
        self.replace_target_op = [tf.assign(t, e) for t, e in zip(t_params, e_params)] # 更新 target_net 参数，将e的参数赋给t
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('eval_net'):
            ...
            c_names, n_l1, w_initializer, b_initializer = \
                ...
                tf.random_normal_initializer(0., 0.3), tf.constant_initializer(0.1)  # tf.random_normal_initializer：正态分布
        ...
        # ---------------- 创建 target 神经网络(很久以前的参数), 提供 target Q ---------------------
    ...
    def store_transition(self, s, a, r, s_):
        # hasattr() 函数用于判断对象是否包含对应的属性。 asattr(object, name)
